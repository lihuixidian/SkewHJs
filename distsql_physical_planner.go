...
var ReplicaOraclePolicy = replicaoracle.RandomChoice
...
func (dsp *DistSQLPlanner) PartitionSpans(
	...
	if planCtx.isLocal {
		...
		partitions = append(partitions,
			SpanPartition{dsp.gatewayNodeID, spans, nil})
...
func (dsp *DistSQLPlanner) planTableReaders(
	...
	if planCtx.isLocal {
		spanPartitions = []SpanPartition{{dsp.gatewayNodeID, info.spans, nil}}
	...
	for i, sp := range spanPartitions {
		...
		if sp.Ranges == nil {
			...
			for j := range sp.Spans {
				tr.Spans = append(tr.Spans, execinfrapb.TableReaderSpan{Span: sp.Spans[j], Ranges: sp.Ranges[j]})
			}
		}
...
func (dsp *DistSQLPlanner) planJoiners(
	...
	hjWorkArgs := MakeDecisionForHashJoin(
		dsp, info.joinNode, p.Processors, leftRouters, rightRouters,
	...
	p.AddJoinStageWithWorkArgs(
		...
		leftRouters, rightRouters, info.joinResultTypes,hjWorkArgs)
	//p.AddJoinStage(
	//	nodes, info.makeCoreSpec(), info.post,
	//	info.leftEqCols, info.rightEqCols,
	//	info.leftPlan.GetResultTypes(), info.rightPlan.GetResultTypes(),
...
func (dsp *DistSQLPlanner) createPhysPlan(
	...
	node, ok := plan.planNode.(*joinNode)
	if ok {
