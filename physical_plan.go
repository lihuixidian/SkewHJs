...
type PRPDWorkArgs struct {
	LeftHeavyHitters 	[]execinfrapb.OutputRouterSpec_MixHashRouterRuleSpec_HeavyHitter
	RightHeavyHitters 	[]execinfrapb.OutputRouterSpec_MixHashRouterRuleSpec_HeavyHitter
}

type PnRWorkArgs struct {
	LeftLocalHeavyHitters 	[]execinfrapb.OutputRouterSpec_MixHashRouterRuleSpec_HeavyHitter
	LeftRandomHeavyHitters 	[]execinfrapb.OutputRouterSpec_MixHashRouterRuleSpec_HeavyHitter
	LeftMirrorHeavyHitters 	[]execinfrapb.OutputRouterSpec_MixHashRouterRuleSpec_HeavyHitter
	RightRandomHeavyHitters 	[]execinfrapb.OutputRouterSpec_MixHashRouterRuleSpec_HeavyHitter
	RightMirrorHeavyHitters 	[]execinfrapb.OutputRouterSpec_MixHashRouterRuleSpec_HeavyHitter
}

type HashJoinWorkArgs struct {
	HJType hashJoinType

	BaseHashArgs  *BaseHashWorkArgs
	PRPDArgs 			*PRPDWorkArgs
	PnRArgs 			*PnRWorkArgs
}

// AddJoinStageWithWorkArgs adds join processors at each of the specified nodes, and wires
...
func (p *PhysicalPlan) AddJoinStageWithWorkArgs(
	...
	if len(nodes) > 1 {
		...
		case PRPD:
			// Set up the left routers.
			leftRules := make([]execinfrapb.OutputRouterSpec_MixHashRouterRuleSpec, 2)
			leftRules[0] = execinfrapb.OutputRouterSpec_MixHashRouterRuleSpec{
				MixHashType: execinfrapb.OutputRouterSpec_MixHashRouterRuleSpec_HASH_LOCAL,
				SkewData: hjWorkArgs.PRPDArgs.LeftHeavyHitters,
			}
			leftRules[1] = execinfrapb.OutputRouterSpec_MixHashRouterRuleSpec{
				MixHashType: execinfrapb.OutputRouterSpec_MixHashRouterRuleSpec_HASH_MIRROR,
				SkewData: hjWorkArgs.PRPDArgs.RightHeavyHitters,
			}
			for _, resultProc := range leftRouters {
				p.Processors[resultProc].Spec.Output[0] = execinfrapb.OutputRouterSpec{
					Type:        execinfrapb.OutputRouterSpec_BY_MIX_HASH,
					HashColumns: leftEqCols,
					MixHashRules: leftRules,
				}
			}
			// Set up the right routers.
			rightRules := make([]execinfrapb.OutputRouterSpec_MixHashRouterRuleSpec, 2)
			rightRules[0] = execinfrapb.OutputRouterSpec_MixHashRouterRuleSpec{
				MixHashType: execinfrapb.OutputRouterSpec_MixHashRouterRuleSpec_HASH_LOCAL,
				SkewData: hjWorkArgs.PRPDArgs.RightHeavyHitters,
			}
			rightRules[1] = execinfrapb.OutputRouterSpec_MixHashRouterRuleSpec{
				MixHashType:execinfrapb.OutputRouterSpec_MixHashRouterRuleSpec_HASH_MIRROR,
				SkewData: hjWorkArgs.PRPDArgs.LeftHeavyHitters,
			}
			for _, resultProc := range rightRouters {
		...
		case PnR:
			// Set up the left routers.
			leftRules := make([]execinfrapb.OutputRouterSpec_MixHashRouterRuleSpec, 3)
			leftRules[0] = execinfrapb.OutputRouterSpec_MixHashRouterRuleSpec{
				MixHashType: execinfrapb.OutputRouterSpec_MixHashRouterRuleSpec_HASH_LOCAL,
				SkewData: hjWorkArgs.PnRArgs.LeftLocalHeavyHitters,
			}
			leftRules[1] = execinfrapb.OutputRouterSpec_MixHashRouterRuleSpec{
				MixHashType: execinfrapb.OutputRouterSpec_MixHashRouterRuleSpec_HASH_AVERAGE,
				SkewData: hjWorkArgs.PnRArgs.LeftRandomHeavyHitters,
			}
			leftRules[2] = execinfrapb.OutputRouterSpec_MixHashRouterRuleSpec{
				MixHashType: execinfrapb.OutputRouterSpec_MixHashRouterRuleSpec_HASH_MIRROR,
				SkewData: hjWorkArgs.PnRArgs.LeftMirrorHeavyHitters,
			}
			for _, resultProc := range leftRouters {
				p.Processors[resultProc].Spec.Output[0] = execinfrapb.OutputRouterSpec{
					Type:        execinfrapb.OutputRouterSpec_BY_MIX_HASH,
			...
			rightRules[0] = execinfrapb.OutputRouterSpec_MixHashRouterRuleSpec{
				MixHashType: execinfrapb.OutputRouterSpec_MixHashRouterRuleSpec_HASH_MIRROR,
				SkewData: hjWorkArgs.PnRArgs.RightMirrorHeavyHitters,
			}
			rightRules[1] = execinfrapb.OutputRouterSpec_MixHashRouterRuleSpec{
				MixHashType: execinfrapb.OutputRouterSpec_MixHashRouterRuleSpec_HASH_AVERAGE,
				SkewData: hjWorkArgs.PnRArgs.RightRandomHeavyHitters,
			}
			for _, resultProc := range rightRouters {
				p.Processors[resultProc].Spec.Output[0] = execinfrapb.OutputRouterSpec{
					Type:        execinfrapb.OutputRouterSpec_BY_MIX_HASH,
