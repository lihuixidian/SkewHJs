...
func makeRouter(
	...
	case execinfrapb.OutputRouterSpec_BY_MIX_HASH:
...
type mixHashRule struct {
	...
	mixHashType execinfrapb.OutputRouterSpec_MixHashRouterRuleSpec_MixHashType
}

func (mhr *mixHashRule) init() error {

	switch mhr.mixHashType {
	case execinfrapb.OutputRouterSpec_MixHashRouterRuleSpec_HASH_MIRROR:
	case execinfrapb.OutputRouterSpec_MixHashRouterRuleSpec_HASH_LOCAL:
		if mhr.localStreamIdx == -1 {
			return errors.Errorf("HASH_LOCAL of mix hash rule has not local stream")
		}
	case execinfrapb.OutputRouterSpec_MixHashRouterRuleSpec_HASH_AVERAGE:
...
var _ execinfra.RowReceiver = &mixHashRouter{}
...
func (mhr *mixHashRouter) Push(
	...
	if err != nil {
		mhr.fwdMetadata(&execinfrapb.ProducerMetadata{Err: err})
		atomic.StoreUint32(&mhr.aggregatedStatus, uint32(execinfra.ConsumerClosed))
		return execinfra.ConsumerClosed
	}
	return aggStatus
}

func (mhr *mixHashRouter) getKey(row rowenc.EncDatumRow) (int64, error) {
	col := mhr.hashCols[0]
	if int(col) >= len(row) {
		err := errors.Errorf("hash column %d, row with only %d columns", col, len(row))
		return -1, err
...
// computeDestination hashes a row and returns the index of the output stream on
// which it must be sent.
func (mhr *mixHashRouter) computeDestination(row rowenc.EncDatumRow) (int, error) {
	...
	for _, col := range mhr.hashCols {
		...
		var err error
		mhr.buffer, err = row[col].Fingerprint(context.TODO(), mhr.types[col], &mhr.alloc, mhr.buffer, nil)
		if err != nil {
			return -1, err
		}
	}
